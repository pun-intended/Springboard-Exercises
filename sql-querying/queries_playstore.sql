...
SELECT app_name, rating, reviews FROM analytics ORDER BY rating LIMIT 5;

--5. Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, rating FROM analytics WHERE rating > 4.8 ORDER BY reviews desc LIMIT 1;

--6. Find the average rating for each category ordered by the highest rated to lowest rated.
SELECT category, AVG(rating) as avg_rtg FROM analytics GROUP BY category ORDER BY AVG(rating) desc;
...
SELECT app_name, reviews FROM analytics WHERE price >.10 AND price < 1 ORDER BY rating desc LIMIT 10;
...
SELECT app_name, min_installs, min_installs/reviews AS proportion FROM analytics WHERE min_installs >= 100000 ORDER BY proportion desc LIMIT 1;
