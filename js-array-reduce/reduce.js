...
function extractValue(arr, key) {
    ...
    return arr.reduce(function(nameArray, val){
        nameArray.push(val[key])
        return nameArray;
    }, [])
}
...
function vowelCount(str) {
    ...
    function isVowel(char){
        charIdx = 'aeiouAEOIU'.indexOf(char);
        return charIdx >= 0;
    }
    return charArray.reduce(function(vowelCounter, char){
        if (isVowel(char)){
            if(vowelCounter[char] == null){
                vowelCounter[char] = 1
            } else {
                vowelCounter[char] += 1;
        }}
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(namesWithTitles, val) {
        val[key] = value;
        namesWithTitles.push(val)
...
array with two arrays inside of it. The partition function should run the callback 
...
function partition(arr, callback) {
    return arr.reduce(function(partition, val){
        if (callback(val)){
            partition[0].push(val);
        } else {
            partition[1].push(val);
        }
        return partition;
    },[[], []])
}
