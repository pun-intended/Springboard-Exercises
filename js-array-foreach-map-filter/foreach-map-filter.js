...
function doubleValues(arr){
    let doubleArr = arr.map(function(item){
        return item*2;
    })
    return doubleArr;
}
...
function onlyEvenValues(arr){
    evenArr = arr.filter(function(item){
        return item % 2 == 0;
    })
    return evenArr;
}
...
function showFirstAndLast(arr){
    function firstLast(str){
        ...
        newStr += charArray[0] + charArray[charArray.length - 1];
        return newStr;
    }
    return arr.map(firstLast);
    
    //map, new function
    // take string, split, take first letter, last letter, return created string
}

/*
Write a function called addKeyAndValue which accepts an array of objects, a key, and 
a value and returns the array passed to the function with the new key and value added 
for each object 

Examples:
    addKeyAndValue([{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}], 
    'title', 'instructor') 
...
function addKeyAndValue(arr,key,value){
    // take first item, for x in y, x["item2"] = item3
    KVmap = arr.map(function(item){
        item[key] = value;
        return item;
    })
    return KVmap;
...
function valTimesIndex(arr){
    indexProduct = arr.map(function(item, index) {
        return item * index;
    })
    return indexProduct;
}
...
a new array with the value of the key with a name of "first" and the value of a key 
...
Examples:
    ...
    {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele"}]) // ['Elie Schoppik', 
    'Tim Garcia', 'Matt Lane', 'Colt Steele']
*/

function extractFullName(arr){
    fullNameArray = arr.map(function(item){
...
Examples:
    ...
    isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", 
    isCatOwner: true}], 'isCatOwner') // [{first: 'Tim', last:"Garcia", isCatOwner: true}, 
    {first: 'Colt', last:"Steele", isCatOwner: true}]
*/

function filterByValue(arr, key) {
    filteredArray = arr.filter(function(obj){
        return obj[key];
    })
...
function find(arr, searchValue) {
    itemIndex = arr.indexOf(searchValue);
...
Examples:
    findInObj([{first: 'Elie', last:"Schoppik"}, {first: 'Tim', last:"Garcia", 
    isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", 
    isCatOwner: true}], 'isCatOwner',true) // {first: 'Tim', last:"Garcia", isCatOwner: true}
*/
...
function doubleOddNumbers(arr) {
    oddArr = arr.filter(function(val){
        return val % 2 != 0;
    })
    console.log(oddArr)
    doubleArr = oddArr.map(function(num) {
        return num * 2;
    })
